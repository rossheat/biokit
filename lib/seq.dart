...
class Nucleotides extends Sequence {
  ...
  Map<String, dynamic> translate({revComp = false, startIdx = 0}) {
    String seq = revComp ? complementary(rev: true) : this._seq;

    String aaSeq = '';
    for (var i = startIdx; i < seq.length - 2; i += 3) {
      String codon = seq.substring(i, i + 3);
      aaSeq += this._type == kDNA ? Structs.dnaCodonToAA[codon] : Structs.rnaCodonToAA[codon];
    }
  ...
  Map<String, int> codonFreq({@required String aminoAcid}) {
    ...
    for (var i = 0; i < this._len - 2; i += 3) {
      String codon = this._seq.substring(i, i + 3);
      String fetchedAminoAcid =
          this._type == kDNA ? Structs.dnaCodonToAA[codon] : Structs.rnaCodonToAA[codon];
  ...
  String complementary({bool rev = false}) {
    String compSeq = this
        ._seq
        .split('')
        .map((nuc) => this._type == kDNA ? Structs.dnaCompNucs[nuc] : Structs.rnaCompNucs[nuc])
        .join();
...
class DNA extends Nucleotides {
  ...
  String transcribe() => this._seq.replaceAll('T', 'U');
  ...
  Map<String, List<Map<String, int>>> restrictionSites({
    ...
    for (String restSeq in restSeqs) {
      ...
      for (RegExpMatch match in matches) {
        int startIdx = this._seq.length - (restSeq.length + match.start);
        int endIdx = startIdx + restSeq.length;
        if (this._seq.substring(startIdx, endIdx) == restSeq) {
  ...
  double tranRatio({@required DNA oSeq}) {
    ...
    this._seq.split('').asMap().forEach((idx, nuc) {
      if (nuc != oSeq.seq[idx]) {
...
class RNA extends Nucleotides {
  ...
  String revTranscribe() => this._seq.replaceAll('U', 'T');
